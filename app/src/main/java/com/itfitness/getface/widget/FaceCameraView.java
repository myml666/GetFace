...
public class FaceCameraView extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    private Camera.Size previewSize;//设置的预览分辨率
    ...
    private void setCameraParameters() {
        ...
        float temp = 10f;
        previewSize = sizes.get(0);//最佳分辨率(最佳的长宽比与屏幕的长宽比差值最小)
        //确定前面定义的预览宽高是camera支持的，不支持取就更大的
        for (int i = 0; i < sizes.size(); i++) {
            float abs = Math.abs(sizes.get(i).width / sizes.get(i).height - screenWidth / screenHeight);
            if(temp>abs){
                temp = abs;
